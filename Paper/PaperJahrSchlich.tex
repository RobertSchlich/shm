...
This excitation method ensures that the structure vibrates in a natural frequency with little interferences.
After excitation the sensor nodes automatically start measuring acceleration values with a sampling rate of 40\,Hz.
To minimize data traffic, each node performs a fast Fourier transform algorithm after sufficient acceleration samples have been collected.
...
The acceleration samples are converted into the vibration frequencies, and the corresponding magnitudes, of the building.
Each sensor node sends only the values of the predominating frequency, i.e. the frequency with the biggest magnitude.
The values are then stored in a MySQL database.
