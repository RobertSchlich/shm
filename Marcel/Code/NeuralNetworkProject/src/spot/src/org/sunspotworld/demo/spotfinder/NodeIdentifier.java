...
public class NodeIdentifier {
	...
	private String ourAddress;		//eigene Adresse
	private String otherAddress;	//base station Adresse	
	
	//Parameter
	private double learning_rate;
	private int sample_period;
	private int amount_of_data;
	private double learn_lim;
	...
	//eigenschaften des sensors im netzwerk
    private int nodeID;			//index des sensors im netzwerk
    private int nodeCount;		//anzahl aller nodes im netzwerk
    private String[] adresses;	//Liste aller Adressen
    ...
    private boolean foundByHost = false;	//kontrollvariable für gefunden sensor
    private long timeout = 2000;			//zeit für empfangen des signals
	 ...
	 * If the node receives a datagram with its own message, it brakes the loop and reads and sets the parameters: 
	 * ID, portIn, portOut. Opens the datastream connections sendOn and listenOn for the communication with the base station.
	...
	public void sendSignal() throws Exception{
		...
		while(foundByHost!=true){
            ...
            if(message.equals(ourAddress)){
            	otherAddress = dgReceive.getAddress();	//adresse lesen, falls Sensor-Node registriert
            	nodeID = dgReceive.readInt();			//Sensor-Node ID in Host-liste lesen
            	portOut = dgReceive.readInt();			//ein und ausgangsports generieren
            	portIn = portOut + (nodeID+1);
            	...
            	//zufällige Wartezeit für jeden nicht erkannten Sensor-Node
            	Random r = new Random();
	...
	public void testSignal() throws Exception{
		...
		while(true){
			try{
				...
				sendOnOutputStream.writeUTF("test-message from node" + (nodeID+1) + ", address: " + ourAddress);
	...
	/**
	 * Receives the list of addresses of all nodes (also the own one) from the base station over the datastream connection.
	...
	public void receiveAdresses() throws IOException{
		...
		nodeCount = listenOnInputStream.readInt();
		//adressliste initialisieren
		adresses = new String[nodeCount];
	...
	/**
	 * @return Array that contains the addresses of all nodes of the network
	 */
